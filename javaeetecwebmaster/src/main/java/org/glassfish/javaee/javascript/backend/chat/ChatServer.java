...
public class ChatServer {
    ...
    public void onOpen(Session peer) {
        logger.log(Level.INFO, "Sessão Aberta: {0}", peer);
        peers.add(peer);
    }

    @OnClose
    public void onClose(Session peer) {
        logger.log(Level.INFO, "Sessão encerrada: {0}", peer);
        peers.remove(peer);
    }

    @OnMessage
    public void onMessage(@Valid ChatMessage message, Session session) {
        logger.log(Level.INFO, "Mensagem recebida {0} do canal {1}",
                new Object[]{message, session});

        for (Session peer : peers) {
            try {
                logger.log(Level.INFO, "Ocorreu broadcasting da mensagem {0} no canal {1}",
                        new Object[]{message, peer});

                peer.getBasicRemote().sendObject(message);
            } catch (IOException | EncodeException ex) {
                logger.log(Level.SEVERE, "Erro enviando mensagem", ex);
            }
        }
    }
